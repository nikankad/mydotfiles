...
static const char dwm[] = ".config/dwm";
...
runautostart(void)
{
	...
	// 	pathpfx = ecalloc(1, strlen(xdgdatahome) + strlen(dwm) + 2);

	// 	if (sprintf(pathpfx, "%s/%s", xdgdatahome, dwm) <= 0) {
	// 		free(pathpfx);
	// 		return;
	// 	}
	// } else {
	...
	// 	if (sprintf(pathpfx, "%s/%s/%s", home, localshare, dwm) < 0) {
	// 		free(pathpfx);
	// 		return;
	// 	}
	// }
	...
	// 	 */
	// 	char *pathpfx_new = realloc(pathpfx, strlen(home) + strlen(dwm) + 3);
	// 	if(pathpfx_new == NULL) {
	// 		free(pathpfx);
	// 		return;
	// 	}
	...
	// 	if (sprintf(pathpfx, "%s/.%s", home, dwm) <= 0) {
	// 		free(pathpfx);
	// 		return;
	// 	}
	// }
...
updatestatus(void)
{
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
		strcpy(stext, "Loading dwmblocks...");
	drawbar(selmon);
}
